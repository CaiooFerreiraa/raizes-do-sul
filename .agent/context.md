...
- [2026-03-30] Bug Fix — Corrigida falha na atualização de perfil: a Server Action `updateProfileAction` não retornava o usuário atualizado, impossibilitando a sincronização imediata no cliente via `update()`. Atualizada a página `/perfil` para passar os dados corretos à função `update()` do `useSession`, garantindo que o estado global do usuário reflita as mudanças instantaneamente na UI e no preenchimento de formulários de encomenda. Adicionado `router.refresh()` para garantir consistência total entre componentes de cliente e servidor.
- [2026-03-22] Interface — Desativada a opção de Cartão de Crédito no formulário de encomenda a pedido do usuário.
- [2026-07-23] Interface — Reativada a opção de Cartão de Crédito no formulário de encomenda, mantendo a integração AbacatePay em Dev Mode (sandbox). Avisos e resumo do checkout agora se adaptam ao pagamento online por cartão ou ao sinal via PIX.
- [2026-07-23] Bug Fix — Corrigido o fluxo de cartão da AbacatePay: erros da API não são mais ignorados, pedidos sem checkout são removidos, o payload V2 passou a usar `imageUrl`, a resposta da API é validada e o redirecionamento para a URL segura ocorre imediatamente. Identificada chave sandbox local inválida/inativa, que precisa ser renovada antes do teste integrado.
- [2026-07-23] Validação — Nova chave confirmada na API V2 com resposta `200`; checkout mais recente validado com URL segura e `devMode: true`. O tratamento de erros agora só cria um produto após `404` e interrompe imediatamente erros de autenticação, evitando requisições e logs duplicados.
- [2026-07-23] Bug Fix — Campos da página de perfil convertidos de `defaultValue` para estado controlado, eliminando o aviso do Base UI `FieldControl` quando a sessão é atualizada após salvar o perfil.
- [2026-07-23] Auth Fix — Login migrado da Server Action com redirecionamento antecipado para `signIn` client-side com `redirect: false`. O `SessionProvider` agora é atualizado antes de `router.replace("/")`, com reload automático de fallback, fazendo o Header reconhecer o usuário imediatamente sem F5 manual.
